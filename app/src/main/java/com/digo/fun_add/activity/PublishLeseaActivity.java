...
import com.digo.fun_add.presenter.PublishLeseaPresenter;
...
public class PublishLeseaActivity extends BaseActivity<PublishLeseaPresenter> implements PublishLeseaView {

    //"租借标题"EditText
    @Bind(R.id.et_lesea_name)
    EditText mEtLeseaName;
    //"开始时间"RelativeLayout
    ...
    //"添加图片"标签TextView
    @Bind(R.id.tv_publish_lesea_add_photo)
    TextView mAddPhotoTextView;
    //RecycleView
    @Bind(R.id.recycleview_publish_lesea)
    ...
    public BasePresenterInfc getPresenter() {
        return new PublishLeseaPresenter(this,new PublishLeseaModel());
    }
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            case R.id.ry_start_time:
                imm.hideSoftInputFromWindow(mEtLeseaName.getWindowToken(), 0);
