package com.digo.utils.timerpickerutils;
...
public class WheelView extends View {
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        textColorCenter = getResources().getColor(R.color
                .pickerview_wheelview_textcolor_center);
        dividerColor = getResources().getColor(R.color
                .pickerview_wheelview_textcolor_divider);
    ...
    void smoothScroll(ACTION action) {
        ...
        mFuture = mExecutor.scheduleWithFixedDelay(new SmoothScrollTimerTask(this,
                mOffset), 0, 10, TimeUnit.MILLISECONDS);
    }
    ...
    public final void setOnItemSelectedListener(OnItemSelectedListener
    ...
    protected void onDraw(Canvas canvas) {
        ...
        while (counter < itemsVisible) {
            ...
            //判断是否循环，如果是循环数据源也使用相对循环的position获取对应的item
            // 值，如果不是循环则超出数据源范围使用""空白字符串填充，在界面上形成空白无数据的item项
            if (isLoop) {
        ...
        //单位的Label
        if (label != null) {
            int drawRightContentStart = measuredWidth - getTextWidth(paintCenterText,
                    label);
            //靠右并留出空隙CENTERCONTENTOFFSET
            canvas.drawText(label, drawRightContentStart+5 , centerY,
                    paintCenterText);
        }
        counter = 0;
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    canvas.restore();
                } else if (translateY <= secondLineY && maxTextHeight + translateY >=
                        secondLineY) {
                    // 条目经过第二条线
                    canvas.save();
                    ...
                    canvas.restore();
                    canvas.save();
                    canvas.clipRect(0, secondLineY - translateY, measuredWidth, (int)
                            (itemHeight));
                    canvas.scale(1.0F, (float) Math.sin(radian) * SCALECONTENT);
                    canvas.drawText(contentText, drawOutContentStart, maxTextHeight,
                            paintOuterText);
                    canvas.restore();
                } else if (translateY >= firstLineY && maxTextHeight + translateY <=
                        secondLineY) {
                    // 中间条目
